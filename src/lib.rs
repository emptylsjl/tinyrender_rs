...
fn triangle(v012: [V3t<i32>; 3], rctx: &Ctx, dim: &Dim, zbuf: &mut [u8], l: usize){
    let [mut v0, mut v1, mut v2] = v012;
    let [w, h] = [dim.w as i32, dim.h as i32];

    if v1.x > v0.x { swap(&mut v0, &mut v1); }
    if v2.x > v0.x { swap(&mut v2, &mut v0); }
    if v2.x > v1.x { swap(&mut v2, &mut v1); }
...
fn triangle2(v012: [V3t<i32>; 3], t012: [V3t<i32>; 3], rctx: &Ctx, vtt: &Ctx, dim: &Dim, zbuf: &mut [u8]){
    let [mut v0, mut v1, mut v2] = v012;
    ...
    for i in 0..x02l {
        ...
        for y in 0..vy11l {
            let (vx, vy) = (vx02, (vy11+y*vyneg));
            if vx < w && vy< h && zbuf[(vx*w+vy) as usize] < z {
                let dt = &vtt.get_image_data(tx02 as f64, (ty11+(y*ty11l*tyneg/vy11l)) as f64, 1., 1.).unwrap();
                // console::log_1(&tx02.into());
...
#[wasm_bindgen]
pub fn rload_obj(obj_str: String) -> Module{
    Module::new(obj_str)
}

#[wasm_bindgen]
pub fn draw(md: Module, rctx: &Ctx, opt: String) -> Result<(), JsValue>{
    let opt: Vec<Trans> = serde_json::from_str(&opt).unwrap();
    let mut stat = Stat::default();
    let t = transform(&opt);
    let c = rctx.canvas().unwrap();
    let d = if c.width() > c.height() {c.height()} else { c.width() };
    ...
    for f in md.f() {
        ...
        let front = light_dir.dot((v[0]- v[2]).v3t().cross((v[1]- v[2]).v3t()).normalize().scale(-1.));

        if front > 0f64 {
            let l = (front * 255.) as usize;
            let v012i = v.map(|x| p3i(&x, &vdim));
            // rctx.set_fill_style(&format!("#{l:x}{l:x}{l:x}FF").into());
            triangle(v012i, rctx, &vdim, &mut zbuf, l);
        } else {
            stat.draw_count += 1;
        }
    }
    console::log_1(&stat.draw_count.into());
    Ok(())
}

#[wasm_bindgen]
pub fn draw2(md: Module, rctx: &Ctx, ctxt: &Ctx, opt: String) -> Result<(), JsValue>{
    let opt: Vec<Trans> = serde_json::from_str(&opt).unwrap();
    let mut stat = Stat::default();

    let t = transform(&opt);
    let vc = rctx.canvas().unwrap();
    let tc = ctxt.canvas().unwrap();
    let d = if vc.width() > vc.height() {vc.height()} else { vc.width() };
    let [ttvx, ttvy] = [tc.width() as f64, tc.height() as f64];
    let vdim = Dim {w: d, h: d, d: 255};
    ...
    for f in md.f() {
        ...
        if front > 0f64 {
            ...
            triangle2(v012, t012, rctx, ctxt, &vdim, &mut zbuf);
        } else {
            stat.draw_count += 1;
        }
    }
